use super::fs::{FsTree, IdenticalSnapshot, Latest};
...
impl<P, S: IndexedFull> RusticFS<P, S> {
    pub(crate) fn from_node(repo: Repository<P, S>, node: Node) -> anyhow::Result<Self> {
        Self::new(repo, FsTree::RusticTree(node.subtree.unwrap()))
    }
    ...
    pub(crate) fn from_snapshots(
        ...
        let root = FsTree::from_snapshots(
            snapshots,
            path_template,
            time_template,
            Latest::AsLink,
    ...
    pub(crate) fn new(repo: Repository<P, S>, root: FsTree) -> anyhow::Result<Self> {
